...
if ($pdo) {
    ...
    // Kommentar: Skapar en lista med användare och ID
    ...
    // Kommentar: Instruktion för alla variabler inom {}
    if (isset($_POST['submit'])) {
        $user = $_POST['author']; //Kommentar: Namn på författaren till inlägget
        $headline = $_POST['title']; //Kommentar: Titeln på inlägget
        $headline = trim($headline);

        $slug = slugify($headline); //Kommentar: Göra $headline till en slug under variabeln $slug

        $text = $_POST['message']; //Kommentar: Samlar alla inläggets delar
